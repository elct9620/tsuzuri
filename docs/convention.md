...
單獨的分詞藏起了角色，讀者分不出它是查詢、尋找、問句還是動作。動詞後面接的分詞則只描述狀態，可以用。
...
先看同一個模組裡的同類名稱，照它們遵守的規則取名。模組裡的規則不一致時，先把模組修正。

| 情境 | 做法 |
|---|---|
| 加入新名稱 | 列出同一模組的同類名稱，照相同規則取名 |
| 同類名稱規則不一致 | 先統一該模組，再加入新名稱 |
| 兩個東西同名 | 只在同一個檔案相遇時才需要區分，否則依角色取名 |
